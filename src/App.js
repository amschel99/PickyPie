...
const App = () => {
    ...
    const fethData = async () => {
      try {
        const response = await fetch(
          `https://www.dictionaryapi.com/api/v3/references/ithesaurus/json/${stateWord}?key=${API_KEY}`
        );
  ...
  return (
    <>
      <AppContext.Provider
        value={{ type, stateWord, definition, synonyms, error }}
      >
        <button
          onClick={() => {
            setStateWord(word.current.accessWord());
          }}
        >
          Search
        </button>
